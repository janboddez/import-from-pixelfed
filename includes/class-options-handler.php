...
?>
		<div class="wrap">
				...
				<table class="form-table">
					<tr valign="top">
						...
						<td><input type="url" id="import_from_pixelfed_settings[pixelfed_host]" name="import_from_pixelfed_settings[pixelfed_host]" style="min-width: 40%;" value="<?php echo esc_attr($this->options['pixelfed_host']); ?>" />
							<p class="description"><?php esc_html_e('Your Pixelfed instance&rsquo;s URL.', 'import-from-pixelfed'); ?></p>
						</td>
					</tr>
					<tr valign="top">
						...
						<td>
							...
							<p class="description"><?php esc_html_e('Post type for newly imported statuses.', 'import-from-pixelfed'); ?></p>
						</td>
					</tr>
					<tr valign="top">
						...
						<td>
							...
							<p class="description"><?php esc_html_e('Post status for newly imported statuses.', 'import-from-pixelfed'); ?></p>
						</td>
					</tr>
					<tr valign="top">
						<th scope="row"><?php esc_html_e('Default post category', 'import-from-pixelfed'); ?></th>
						<td>
							<?php wp_dropdown_categories( array(
									'show_option_none'=>'Select category',
									// 'value_field'=>'name',
									'name'=>'import_from_pixelfed_settings[post_category]',
									// 'selected' => in_array(wp_unslash($settings['post_category']), get_categories())
					...
					<tr valign="top">
						...
						<p class="description"><?php _e('Import only statuses with <strong>any</strong> of these (comma-separated) tags. (Leave blank to import all statuses.)', 'import-from-pixelfed'); // phpcs:ignore WordPress.Security.EscapeOutput.UnsafePrintingFunction 
												?></p></td>
					</tr>
					//-->
					<tr valign="top">
						...
						<td><textarea id="import_from_pixelfed_settings[denylist]" name="import_from_pixelfed_settings[denylist]" style="min-width: 40%;" rows="5"><?php echo esc_html($this->options['denylist']); ?></textarea>
							<p class="description"><?php esc_html_e('Ignore statuses with these words (case-insensitive).', 'import-from-pixelfed'); ?></p>
						</td>
					</tr>
				</table>
			...
			<?php
			// To do: move all of this to a function of its own, simplify.
...
<?php
	...
	{
		if (isset($_GET['reset']) && 'true' === $_GET['reset'] && isset($_GET['_wpnonce']) && wp_verify_nonce(sanitize_key($_GET['_wpnonce']), 'import-from-pixelfed-reset')) {
	...
	/**
	 * Verifies Import From Pixelfed's token status.
	 *
	 * Normally runs once a day.
	 */
